...
impl<'a> ToLower<Block<StrType<'a>>, &'a mut IdInfo> for Side<'a> {
    fn into_lower(self) -> Block<StrType<'a>> {
        state.max_side_id += 1;
        let id = state.max_side_id;
        let props = vec![
...
// impl<'a> ToLower<Vmf<StrType<'a>>, &'a mut IdInfo> for Map<'a> {
//     fn into_lower(self, state: &'a mut IdInfo) -> Vmf<StrType<'a>> {
//         let mut vmf = Vmf::default();
//         let mut solid_blocks: Vec<_> = self.solids.iter().map(|s| s.to_lower(state)).collect();
//         if let Some(cordon) = &self.options.dev_skybox {
//             solid_blocks.extend(
//                 Room::new(cordon.clone()).construct_sky().into_iter().map(|s| s.to_lower(state)),
//             );
//         }
...
//         if let Some(cordon) = &self.options.dev_skybox {
//             solids.extend(
//                 Room::new(cordon.clone()).construct_sky().into_iter().map(|s| s.to_block(state)),
//             );
//         }
//         vmf.push(Block {
